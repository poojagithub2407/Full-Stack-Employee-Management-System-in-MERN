import { ArrowRightIcon, CalendarIcon, DollarSign, DollarSignIcon, FileTextIcon } from 'lucide-react';
     ...
     return (
          <div className='animate-fade-in'>
               ...
               <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8'>
                    {
                         cards.map((card, index) => (
                              <div key={index} className="card card-hover p-5 sm:p-6 relative overflow-hidden group flex items-center justify-between">
                                   ...
                                   <card.icon className='size-10 p-2.5 rounded-lg bg-slate-100 text-slate-600 group-hover:bg-indigo-500/70 group-hover:text-indigo-600 transition-colors duration-200' />
                              </div>
                         ))
                    }
               </div>
               <div className='flex flex-col sm:flex-row gap-3'>
                    ...
                    <Link to='/leave' className='btn-secondary text-center inline-flex items-center justify-center gap-2'>Apply for Leave</Link>
               </div>
          </div>
     )
}
